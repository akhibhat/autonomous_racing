...
class ScanProcessor
{
    ...
    public:
        ...
        {
            // First we read the laser scan and store all important variables.
            // The scan points are stored in the variable points
            readScan(msg);

            // For first scan we have nothing to compare to
            ...
            while (count < MAX_ITER && (curr_trans != prev_trans || count == 0))
            {
                ...
                // First we transform the new points into the frame of the previous points
                transformPoints(points, curr_trans, transformed_points);
            
                // We then try to find correspondences between the two sets of points
                ...
                // Then we update the transform for the next iteration
        ...
        void publishPos()
        {
            ...
            tf::Matrix3x3 tf3d;
            tf3d.setValue(static_cast<double>(global_tf(0,0)), static_cast<double>(global_tf(0,1)), 0, static_cast<double>(global_tf(1,0)), static_cast<double>(global_tf(1,1)), 0, 0, 0, 1);
